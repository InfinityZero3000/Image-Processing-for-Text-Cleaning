    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-4 space-y-3">
        ...
        <button 
          ...
          disabled={isProcessing || !hasProcessedImage || backendStatus !== 'online'}
          className={`w-full py-3 px-4 ${themeColor} hover:bg-red-900 text-white rounded-xl font-bold shadow-lg active:scale-95 transition-all flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed`}
          title={backendStatus !== 'online' ? 'Yêu cầu Backend đang hoạt động' : 'So sánh OCR giữa ảnh gốc và ảnh đã xử lý'}
        >
          <Wand2 size={18} />
          <span>So sánh OCR (Gốc vs Xử lý)</span>
        </button>
