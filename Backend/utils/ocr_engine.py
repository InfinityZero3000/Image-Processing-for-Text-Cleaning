...
class OCREngine:
    ...
    def __init__(self):
        # Tesseract configuration - sử dụng multiple PSM modes để tối ưu
    ...
    def _preprocess_for_ocr(self, image):
        ...
        # Resize nếu ảnh quá nhỏ (tăng độ phân giải giúp OCR tốt hơn)
    ...
    def _run_tesseract(self, image, lang='vie') -> Dict[str, Any]:
        """Chạy Tesseract với nhiều PSM modes và chọn kết quả tốt nhất"""
        ...
        for psm in psm_modes:
            try:
                ...
                # Chọn kết quả có confidence cao nhất và có text
        ...
        # Nếu không có kết quả nào tốt, thử lại với ảnh gốc
        if not best_result or best_confidence < 20:
            config = '--oem 3 --psm 6'
            best_result = pytesseract.image_to_string(pil_image, lang=lang, config=config).strip()
