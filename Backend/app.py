...
def process_image():
    ...
    try:
        ...
        try:
            ...
            # Nếu là lỗi Hugging Face quota, fallback sang AI Local hoặc Premium
            if 'GPU quota' in error_msg or 'exceeded' in error_msg.lower():
                logger.warning(f"AI Cloud failed due to quota: {error_msg}. Falling back to Premium pipeline.")
                result = image_processor.process_pipeline_premium(image, settings)
                result['warning'] = 'AI Cloud hết GPU quota. Đã tự động chuyển sang Premium pipeline.'
            else:
        ...
        return jsonify({
            ...
            'warning': result.get('warning'),  # Thêm warning nếu có fallback
            'stats': {
...
def compare_ocr():
    """
    So sánh kết quả OCR giữa ảnh gốc và ảnh đã xử lý
    Input JSON: {
        original: base64 image,
        processed: base64 image,
        lang: 'vie' or 'eng',
        provider: 'tesseract' (default)
    }
    """
    try:
        data = request.json
        ...
        start_time = datetime.now()
        ocr_original = ocr_engine.extract_text(original, lang=lang, provider=provider)
        ocr_processed = ocr_engine.extract_text(processed, lang=lang, provider=provider)
        total_time = (datetime.now() - start_time).total_seconds() * 1000
        
        # Calculate comparison metrics
        text_original = ocr_original.get('text', '')
        text_processed = ocr_processed.get('text', '')
        conf_original = ocr_original.get('confidence', 0)
        conf_processed = ocr_processed.get('confidence', 0)
        
        # Character count comparison
        chars_original = len(text_original)
        chars_processed = len(text_processed)
        chars_diff = chars_processed - chars_original
        chars_diff_percent = (chars_diff / chars_original * 100) if chars_original > 0 else 0
        
        # Word count comparison
        words_original = len(text_original.split())
        words_processed = len(text_processed.split())
        words_diff = words_processed - words_original
        words_diff_percent = (words_diff / words_original * 100) if words_original > 0 else 0
        
        # Confidence improvement
        conf_improvement = conf_processed - conf_original
        conf_improvement_percent = (conf_improvement / conf_original * 100) if conf_original > 0 else 0
        
        # Simple similarity check (how many words match)
        words_orig_set = set(text_original.lower().split())
        words_proc_set = set(text_processed.lower().split())
        common_words = words_orig_set.intersection(words_proc_set)
        similarity_percent = (len(common_words) / len(words_orig_set) * 100) if words_orig_set else 0
        
        # Determine if processing improved OCR
        is_improved = (conf_processed > conf_original) or (chars_processed > chars_original)
        ...
        return jsonify({
            ...
            'original': {
                ...
                'characters': chars_original,
                'words': words_original
            },
            'processed': {
                'text': text_processed,
                'confidence': conf_processed,
                'characters': chars_processed,
                'words': words_processed
            },
            'comparison': {
                'confidence_improvement': round(conf_improvement, 2),
                'confidence_improvement_percent': round(conf_improvement_percent, 2),
                'characters_diff': chars_diff,
                'characters_diff_percent': round(chars_diff_percent, 2),
                'words_diff': words_diff,
                'words_diff_percent': round(words_diff_percent, 2),
                'similarity_percent': round(similarity_percent, 2),
                'is_improved': is_improved,
                'verdict': 'IMPROVED' if is_improved else 'NO_CHANGE' if conf_improvement == 0 else 'DEGRADED'
            },
            'time': round(total_time, 2)
        })
        
    except Exception as e:
        logger.error(f"Error comparing OCR: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
